...
This page documents TRAIN's custom flow for integrations with the Aztec network. It is part of TRAIN's Custom Flows collection — adaptations of the core atomic-swap pattern required by particular chains or environments. The Aztec network is privacy-preserving: user identity remains private, while balances, commitments, and HTLC state are publicly visible via protocol logs. These public logs provide the signals solvers need to coordinate swaps without revealing user identity. The following sections describe actors, core concepts, and the logical flows for the Aztec network.
...
- User: the party moving value.
 ...
 - HTLC / TRAIN contract: per-chain contracts enforcing hashlock and timelock commitments.
