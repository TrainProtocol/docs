...
<Steps>
  ...
  <Step title="dApp Deploy">
    Create a pull request for a [popular bridge UI](https://app.train.tech) or fork and deploy a separate UI.
  </Step>
</Steps>

These steps can be performed independently without needing permission or interaction with any party or entity. At this stage, anyone can transfer assets to the new network. The network-adding process in the TRAIN protocol is **similar to adding an ERC20 token to the Uniswap protocol**, making bridging trust-minimized and permissionless. The detailed implementation steps can be found in the [Solver Implementation](/protocol-spec/implementation-solver) spec.