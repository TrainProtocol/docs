...
By making atomic swaps practical for bridging, we gain a crucial advantage. The same hashlock can be reused across multiple networks (multi-Solver routing) for a single bridging request. Once the secret is revealed, all linked HTLCs in all networks can be unlocked.
