...
<Steps>
  ...
  <Step title="(signature flow) Sign a message instead of adding a lock">
     This step can be used instead of step 3: the user signs a message (addLockSig) containing the `Id`, `hashlock`, and `timelock`, which an authorized party can submit to finalize the lock.
  </Step>
</Steps>
